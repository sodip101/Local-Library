    ...
    (req,res,next)=>{
        ...
        if(!errors.isEmpty()){
            Book.find({},'title').exec(function(err,book_list){
                if(err){return next(err);}
                res.render('bookinstance_form',{title:'Update Bookinstance',book_list:book_list,bookinstance:bookinstance,errors:errors.array()});
            });
        }else{
            BookInstance.findByIdAndUpdate(req.params.id,bookinstance,{},(err,bookinst)=>{
                if(err){return next(err);}
                res.redirect('/catalog/'+bookinst.url);
            })
        }
    }
];